...
✅ ✨ PATCH: fallback OCR si PDF image/scanné + logs
"""
...
def _read_pdf_text(pdf_path: str, debug: bool = True) -> str:
    """
    Lit le texte complet d'un PDF.
    ✅ PATCH: fallback OCR si PDF image/scanné (Eurofins)
    ✅ Logs (pour comprendre pourquoi c'est vide)
    """
    try:
        import pdfplumber
    ...
    text = "\n".join(chunks).strip()
    if debug:
        print(f"[PDF] extract_text length={len(text)} | file={os.path.basename(pdf_path)}")

    # Détection simple PDF image/scan -> OCR
    if len(text) < 50:
        if debug:
            print("[PDF] Texte insuffisant -> tentative OCR...")
        ...
        try:
            ...
            with pdfplumber.open(pdf_path) as pdf:
                for i, page in enumerate(pdf.pages, start=1):
                    ...
                    page_ocr = pytesseract.image_to_string(img, lang="fra")
                    if debug:
                        print(f"[OCR] page {i} length={len(page_ocr.strip())}")
            ...
            if debug:
                print(f"[OCR] indisponible/échec: {type(e).__name__}: {e}")
...
def extract_synlab_biology(pdf_path, progress=None, debug: bool = True):
    """
    ...
    text = _read_pdf_text(pdf_path, debug=debug)
    lines = [ln.strip() for ln in text.splitlines() if ln.strip()]
    out = {}

    if debug:
        print(f"[BIO] lines={len(lines)}")
    ...
    for idx, ln in enumerate(lines):
        ...
        if progress and idx % 10 == 0:
            percent = 15 + int((idx / total_lines) * 15) if total_lines else 15
    ...
    for biomarker_name, data in out.items():
        if not data.get("reference"):
            ...
            if default_ref:
                ...
                data["status"] = determine_biomarker_status(data.get("value"), default_ref, biomarker_name)

    if progress:
        progress.update(30, f"Biologie: {len(out)} biomarqueurs")

    if debug:
        print(f"[BIO] matched={len(out)}")
...
def _extract_bacterial_groups_v2(text):
    ...
    if len(result_matches) == len(STANDARD_GROUPS):
        for i, (group_code, group_name) in enumerate(STANDARD_GROUPS):
            ...
            groups.append({'category': group_code, 'name': group_name, 'abundance': group_status})
    else:
        ...
        if hasattr(page, 'curves'):
            for curve in page.curves:
                if 'pts' in curve and len(curve['pts']) >= 4:
                    ...
                    dots.append({'x': center_x, 'y': center_y, 'type': 'circle'})
            ...
            for rect in page.rects:
                if 'x0' in rect and 'y0' in rect and 'x1' in rect and 'y1' in rect:
                    ...
                    if 2 < width < 10 and 2 < height < 10:
                        ...
                        dots.append({'x': center_x, 'y': center_y, 'type': 'rect'})
...
def extract_idk_microbiome(pdf_path, excel_path=None, enable_graphical_detection=True,
                          resolution=200, progress=None, debug: bool = True):
    try:
        import pdfplumber
    except ImportError as e:
    ...
    if debug:
        print(f"[MICRO] text length={len(text)} lines={len(lines)}")
    ...
    try:
        ...
        for idx, (_, row) in enumerate(df.iterrows()):
            if progress and idx % 5 == 0:
                percent = 10 + int((idx / total_rows) * 20) if total_rows else 10
...
def extract_microbiome_from_excel(excel_path: str) -> Dict[str, Any]:
    """
    Extrait microbiome depuis fichier Excel structuré
    """
    try:
...
def extract_all_data(bio_pdf_path=None, bio_excel_path=None, micro_pdf_path=None,
                     ...
                     show_progress=True, debug: bool = True):
    ...
    if bio_pdf_path:
        biology.update(extract_synlab_biology(bio_pdf_path, progress=progress, debug=debug))

    if bio_excel_path:
        biology.update(extract_biology_from_excel(bio_excel_path, progress=progress))
